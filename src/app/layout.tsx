...
export default function Home() {
  ...
  const send = async () => {
    ...
    try {
      ...
      setLoading(false);
      setMessages(ms => [...ms, { role: "assistant", content: "System error—verify OpenAI key and reload. We rise at 3:58." }]);
    }
  };
    ...
    <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-2xl">
        ...
        <div className="bg-zinc-900 rounded-2xl p-6 min-h-96 border border-zinc-800 mb-4">
          ...
          {loading && <p className="text-zinc-500 text-center">AI deploying...</p>}
        </div>
...
export const metadata: Metadata = {
  title: 'AI Sales Agent - Automated Empire Closer',
  description: 'Qualify leads hard, charge deposits, book calls — 24/7 while you build.',
};
...
html, body { @apply bg-black text-white; }import OpenAI from 'openai';
...
export async function POST(req: NextRequest) {
  ...
  const systemPrompt = `Ruthless high-ticket closer for $15k+ coaching. Qualify brutally: $100k+ budget, decision-maker, sharp pain, 90-day close. Hot lead? Grab name/email/phone → $499 deposit + Calendly. Cocky, direct, zero fluff.`;
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      tools: [{
        ...
        function: {
          ...
          description: 'Trigger deposit + book',
          parameters: {
            type: 'object',
      ...
      const checkoutRes = await fetch('https://api.lemonsqueezy.com/v1/checkouts', {
        ...
        body: JSON.stringify({
          data: {
            type: 'checkouts',
            attributes: {
              product_options: { redirect_url: `${process.env.NEXT_PUBLIC_SITE_URL}/thanks?calendly=${process.env.CALENDLY_LINK}` },
              checkout_data: { email: args.email, custom: { ...args, lead_id: leadId } }
            },
            relationships: {
    ...
    return NextResponse.json({ reply: completion.choices[0].message.content || 'What's your real play?' });
  } catch (error) {
    console.error(error);
    return NextResponse.json({ reply: 'Hit a snag—DM for manual close.' }, { status: 500 });
  }
}
